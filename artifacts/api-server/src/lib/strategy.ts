...
export type AnalysisResult = {
  realSymbol: string;
  currentState: string;
  htfBias: "BULLISH_DISCOUNT" | "BEARISH_PREMIUM" | "BULLISH" | "BEARISH" | "RANGING" | null;
  htfConflict: boolean;
  ...
  for (const [type, candidates] of [
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      const matches = candidates.filter(
        (candidate) =>
          Math.abs(candidate.index - current.index) >= 3 &&
...
/**
 * Closed-candle detection of Fair Value Gaps (FVG) and Order Blocks (OB).
 * Operates strictly on closed candles.
 */
export function inferPoi(
  ...
  // 1. Detect 3-candle Fair Value Gap (FVG) in recent closed candles
  for (let i = closedCandles.length - 2; i >= Math.max(2, closedCandles.length - 15); i--) {
    const candle1 = closedCandles[i - 2];
    const candle2 = closedCandles[i - 1]; // displacement candle
    const candle3 = closedCandles[i];

    // Bullish FVG: candle1 high < candle3 low with strong bullish candle2
    ...
    // Bearish FVG: candle1 low > candle3 high with strong bearish candle2
